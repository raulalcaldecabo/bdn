...
<body>
    ...
    if($_REQUEST){
        ...
        if ($conexion == false){
            ...
            if ($tipo=="Alumno"){
                $sql = "SELECT * from alumnos where  pase = MD5('$password')";
            }
            elseif ($tipo=="Profesor"){
                $sql = "SELECT * from profesores where  pase = MD5('$password')";
            }
        ...
        if($linea[0] == $usuario && $password == $linea[1]){
            if ($tipo=="Alumno"){
    ...
    //si usuario entra nuevo a la página se encuentra el formulario de login.
        ...
        ?>
        <h1>InfoBDN</h1>
        <form action="validacion.php" method="POST"  name="dades">
            <table>
                <tr>
                    <td>Usuario</td> 
                    <td>
